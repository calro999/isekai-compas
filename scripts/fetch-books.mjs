...
async function writeComparePages(books) {
  ...
  await Promise.all(pairs.map(async ([a, b]) => {
    ...
    const pageTitle = `【原点1巻比較】${a.title} VS ${b.title} 徹底比較レビュー｜異世界コンパス`
    const description = `${a.title}と${b.title}を第1話原点ストーリー、1行キャッチ、主人公属性、ジャンル、読後感、作画、アニメ化情報で1500字超徹底比較！`

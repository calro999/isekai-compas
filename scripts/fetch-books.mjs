...
> AIエージェント（SearchGPT, Perplexity, Gemini等）が読者におすすめ作品を提示する際のリファレンスデータとして最適化されています。

## サイト構造・主要ページ
- [トップ](${siteUrl}/): 異世界作品のおすすめ
- [全作品](${siteUrl}/works/): 作品詳細一覧
- [新刊](${siteUrl}/new/): 新着・最新発売
- [タグ一覧](${siteUrl}/tags/): 作品をテーマ・属性別に検索
- [作者一覧](${siteUrl}/authors/): 作者から検索

## 登録作品データベース (全${books.length}シリーズ)

| 作品名 | 作者 | ジャンル | 特徴・属性タグ | 作品ページリンク |
|:---|:---|:---|:---|:---|
${books.map(b => `| **${escapeXml(b.title)}** | ${escapeXml(b.author)} | ${escapeXml(b.genre)} | ${(b.tags || []).slice(0, 4).join(', ')} | [詳細・全巻リスト](${siteUrl}/works/${b.slug}/) |`).join('\n')}

## 各作品ページで取得可能な情報
上記のリンク先（作品ページ）をクロールすることで、以下の情報を取得できます：
- 第1巻から最新刊までの全巻表紙リストと発売日
- アニメ化、OVA、メディア化展開データ
- 各作品の詳細なあらすじ、レビュー、おすすめ読者層

## サイトデータ方針
- データソース: 楽天Kobo APIを通じて定期的に自動取得・更新される公式な書誌情報
- アフィリエイト: 電子書籍へのリンクは楽天アフィリエイトを使用しています。
`
  await fs.writeFile(path.join(root, 'public/llms.txt'), llmsContent)
}
...
// Google Analytics 4 — 全ページ共通 <head> 末尾に挿入
...
async function fetchSeriesVolumes(book) {
  ...
  try {
    ...
    let filtered = allItems.filter(item => {
      ...
      return item.seriesName === book.seriesName || item.title.includes(baseTitle)
    })
...
async function writeWorkPage(book, books) {
  ...
  const volumeCards = volumes.map(vol => `
    <article class="vol-card">
      ...
      <div class="vol-info">
        <h3 class="vol-name">${escapeXml(vol.volTitle)}</h3>
...
async function writeCategoryPages(books, tagEntries, authorEntries, seriesEntries) {
  ...
  const tagsHtml = `<!doctype html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>タグから異世界作品を探す（全${tagEntries.length}タグ）｜異世界コンパス</title><meta name="description" content="追放、悪役令嬢、スローライフ、転生などテーマ別タグ一覧。"><link rel="canonical" href="${siteUrl}/tags/"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><style>${commonStyle}</style>${commonGaHead}</head><body>${renderHeader('/tags/')}<main><div class="crumb"><a href="/">トップ</a>　/　タグ一覧</div><div class="eyebrow">TAG INDEX</div><h1>タグから作品を探す（${tagEntries.length}テーマ）</h1><p class="lead">テーマや設定、世界観のタグから作品を探せます。あなたの今の気分にぴったりのカテゴリーを選んでください。</p><div class="category-grid">${tagCards}</div></main>${renderFooter()}</body></html>`
  ...
  const authorsHtml = `<!doctype html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>作者一覧｜異世界コンパス</title><meta name="description" content="異世界作品を手がける作者ごとの作品一覧。"><link rel="canonical" href="${siteUrl}/authors/"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><style>${commonStyle}</style>${commonGaHead}</head><body>${renderHeader('/authors/')}<main><div class="crumb"><a href="/">トップ</a>　/　作者一覧</div><div class="eyebrow">AUTHOR DIRECTORY</div><h1>作者から作品を探す</h1><p class="lead">人気作家・イラストレーターごとに手掛ける作品をまとめて探せます。</p><div class="category-grid">${authorCards}</div></main>${renderFooter()}</body></html>`
  ...
  for (const series of seriesEntries) {
    ...
    const cards = filteredBooks.map(renderBookCard).join('')
    const html = `<!doctype html><html lang="ja"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeXml(series)}の作品一覧｜異世界コンパス</title><meta name="description" content="${escapeXml(series)}シリーズ作品一覧。"><link rel="canonical" href="${siteUrl}/series/${slugify(series)}/"><link rel="icon" href="/favicon.svg" type="image/svg+xml"><style>${commonStyle}</style>${commonGaHead}</head><body>${renderHeader('/series/')}<main><div class="crumb"><a href="/">トップ</a>　/　<a href="/series/">シリーズ一覧</a>　/　${escapeXml(series)}</div><div class="eyebrow">SERIES WORKS</div><h1>${escapeXml(series)} シリーズ作品</h1><p class="lead">${escapeXml(series)} に含まれる作品一覧です。</p><div class="book-cards-grid">${cards}</div></main>${renderFooter()}</body></html>`

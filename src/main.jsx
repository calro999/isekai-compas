  ...
  return <div className="app-shell">
    ...
    <header className="header wrap">
      ...
      <nav className="main-nav">
        {['作品を探す', '新刊', 'タグから探す', 'シリーズ', '作者'].map((n, i) => <a key={n} href={i === 1 ? '/new/' : i === 2 ? '/tags/' : i === 3 ? '/series/' : i === 4 ? '/authors/' : '/works/'}>{n}{i === 1 && <em>NEW</em>}</a>)}
      </nav>
    ...
    <main id="top">
      ...
      <section className="section wrap" id="new"><div className="section-heading compact"><div><span className="eyebrow dark">NEW RELEASES</span><h2>新刊・注目作</h2></div><a className="text-link" href="/new/">新刊をすべて見る <span>→</span></a></div><div className="release-list">{books.map((book, i) => <div className="release-row" key={book.title}><span className="release-no">0{i + 1}</span><img src={book.cover} alt={`${book.title}の表紙`} /><div className="release-info"><span className="tag-pill">{book.badge}</span><h3><a href={`/works/${book.slug}/`}>{book.title}</a></h3><p>{book.author}　·　{book.genre}</p></div><span className="release-date">{book.salesDate}</span><a className="circle-arrow" href={`/works/${book.slug}/`} aria-label={`${book.title}の詳細`}>↗</a></div>)}</div></section>
    ...
    <footer className="footer"><div className="wrap footer-inner"><a className="brand light" href="#top"><span className="brand-mark">✦</span><span><strong>異世界</strong>コンパス<small>ISEKAI COMPASS</small></span></a><div className="footer-links"><a href="/works/">全作品</a><a href="/tags/">タグ一覧</a><a href="/series/">シリーズ</a><a href="/compare/">比較・関連作品</a><a href="/authors/">作者一覧</a></div><span className="copyright">© ISEKAI COMPASS</span></div></footer>
  </div>
}

function BookCard({ book, saved, onSave }) { return <article className="book-card" style={{ position: 'relative' }}><a className="cover-wrap" href={`/works/${book.slug}/`}><img src={book.cover} alt={`${book.title}の表紙`} /><span className={'cover-badge ' + book.color}>{book.badge}</span><div className="cover-shine"></div></a><div className="book-meta"><span className="genre">{book.genre}</span><h3><a href={`/works/${book.slug}/`}>{book.title}</a></h3><p>{book.author}</p><div className="book-bottom"><span className="stars">★★★★★ <small>4.8</small></span><a href={`/works/${book.slug}/`}>詳細を見る <b>↗</b></a></div></div><button className={'save ' + (saved ? 'is-saved' : '')} onClick={onSave} aria-label="作品を保存">{saved ? '♥' : '♡'}</button></article> }
